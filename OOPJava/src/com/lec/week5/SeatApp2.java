...
//		
//		String[] studentAuto = new String[max]; 
...
//				break;
//================================================5. 콘솔출력===========================================================				
//		case 5: { %2$s %1$s %2$s %1$s %2$s %1$s\n", middleCrossBound, seatSpace,
//						section1_leftSpace);
//				System.out.printf(
...
//				System.out.printf("%3$s %1$s %2$s %1$s
//				System.out.printf("%3$s %1$s %2$s %1$s %2$s %1$s %2$s %1$s %2$s %1$s\n", middleCrossBound, seatSpace,
//						section1_leftSpace);
//				System.out.printf(
//						"%9$s %1$s %4$s %2$s %3$s %1$s %5$s %2$s %3$s %1$s %6$s %2$s %3$s %1$s %7$s %2$s %3$s %1$s %8$s %2$s %3$s\n",
...
//================================================6. 종료하기==========================================================			
	...
	static void inputStudents(Student st) {
		
		System.out.print("학생 수를 입력하세요>>");
		Scanner sc = new Scanner(System.in);
	
		st.pnum = sc.nextInt();
		st.studentsArr = new String[st.pnum];
		
		for (int i = 0; i < st.pnum; i++) {
			System.out.print((i + 1) + "번째학생이름>>\n");
			String name = sc.next();
			st.studentsArr[i] = name;
		
		}
		System.out.println(Arrays.toString(st.studentsArr));
	}
	
	static void shuffle(Student st) {
		String[] randomArr = new String[st.pnum];
		Random rand = new Random();
		
		//랜덤 배열에서 중복숫자 제거
		for (int i = 0; i < randomArr.length; i++) {
			randomArr[i] = Integer.toString(rand.nextInt(st.pnum) + 1);
			for (int j = 0; j < i; j++) {
		...
		System.out.println("학생 이름 배열입니다." + Arrays.toString(st.studentsArr));
		System.out.println();
		
		//학생의 이름을 저장한 studentArr과 randomArr의 인덱스를 매칭시켜서 2차원 배열에 담아준다.
		String[][] matchArr = new String[st.pnum][2];
		for (int i = 0; i < st.pnum; i++) {
			matchArr[i][0] = st.studentsArr[i];
		...
		String[] totalGroup = new String[5];
		
//		StudentGroup[] sg = new StudentGroup[5];
//		 st.sg.group1 = new String[gnum[0]];
//		 for(int i =0; i< gnum[0]; i++)
//			 st.sg.group1[i] = st.studentsArr[i];
//		 
//		 System.out.println(Arrays.toString(st.sg.group1));
//		sg.group1 = new String[gnum[0]];
//		group2 = new String[gnum[1]];			
//		group3 = new String[gnum[2]];
//		group4 = new String[gnum[3]];
//		group5 = new String[gnum[4]];
//		sg[0] = st.group1;
//		
//		int count = 0; // 배열의 크기, 0~학생수만큼 증가하면서 저장된 이름값을 순서대로 받아올 수 있도록
//		
//		for(int j=0; j<5;j++) {
//		if(gnum[i]==0,gnum[i]==3,gnum[i]==4)
//			
//			group1[i] = matchArr[count][0];
//		else if(gnum[i]==0,gnum[i]==3,gnum[i]==4)
//			
//		i++;
//		count++;
//		}
